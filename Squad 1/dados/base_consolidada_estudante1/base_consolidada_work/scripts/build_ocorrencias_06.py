...
META = json.load(open(f"{BASE}/dados/SGB_GeoSGB/recmin/ocorrencias_recursos_minerais_GO.metadados.json", encoding="utf-8"))
...
DATA = {sid: pd.Timestamp(max(os.path.getmtime(a) for p in pads for a in glob.glob(f"{BASE}/{p}")), unit="s").strftime("%Y-%m-%d")
