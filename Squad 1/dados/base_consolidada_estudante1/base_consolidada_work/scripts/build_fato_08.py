...
for _f in FONTES.values():  # mesmo cálculo de data_arquivo_local da 07 (a governança confere)
    _f["data_acesso"] = pd.Timestamp(os.path.getmtime(f"{BASE}/{_f['arquivo']}"), unit="s").strftime("%Y-%m-%d")
TIPO_FONTE = "oficial"
...
def confere_linhas(sid, df, sep):
    raw = open(f"{BASE}/{FONTES[sid]['arquivo']}", "rb").read().decode("cp1252", errors="replace")
...
for sid, METS in (("SRC_ANM_PROD_BRUTA", PB_MET), ("SRC_ANM_PROD_BENEF", PF_MET)):
    df = read_csv(f"{BASE}/{FONTES[sid]['arquivo']}")
