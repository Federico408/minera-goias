...
def info_arquivos(padroes):
    arqs = sorted({a for p in padroes for a in glob.glob(f"{BASE}/{p}")})
    if not arqs:
    ...
    return dict(arquivos=len(arqs), tamanho_mb=round(sum(os.path.getsize(a) for a in arqs) / 1e6, 2),
                ...
                data_arquivo_local=pd.Timestamp(max(os.path.getmtime(a) for a in arqs), unit="s").strftime("%Y-%m-%d"))
...
def csv_local(p):
    s = separador(f"{BASE}/{p}")
    return pd.read_csv(f"{BASE}/{p}", sep=s, encoding="cp1252", encoding_errors="replace", dtype=str,
...
def periodo_json(p):
    try:
        d = json.load(open(f"{BASE}/{p}", encoding="utf-8"))
...
try:  # RECMIN: URL exata da requisição e período vêm do próprio download (não duplicar à mão)
    _META_RECMIN = json.load(open(f"{BASE}/dados/SGB_GeoSGB/recmin/ocorrencias_recursos_minerais_GO.metadados.json", encoding="utf-8"))
    _rc = json.load(open(f"{BASE}/dados/SGB_GeoSGB/recmin/ocorrencias_recursos_minerais_GO.geojson", encoding="utf-8"))
...
def periodo_imb():
    partes = []
    for p in sorted(glob.glob(f"{BASE}/dados/IMB/consulta*.csv")):
        try:
...
def periodo_zip(padroes):
    ...
    partes = []
    for p in padroes:
        for a in sorted(glob.glob(f"{BASE}/{p}")):
            try:
...
def periodo_nazario(p):
    try:
        wbn = openpyxl.load_workbook(f"{BASE}/{p}", read_only=True)
...
def periodo_mineradoras(p):
    try:
        wbm = openpyxl.load_workbook(f"{BASE}/{p}", read_only=True)
...
def checagem_pop_uf():
    ...
    try:
        uf = json.load(open(f"{BASE}/dados/IBGE/populacao_goias_ultimo_ano.json", encoding="utf-8"))[1]
        mun = json.load(open(f"{BASE}/dados/IBGE/populacao_municipal_goias_ultimo_ano.json", encoding="utf-8"))[1:]
...
FONTES = [
    ...
    dict(source_id="SRC_ANM_CADASTRO", nome_fonte="ANM — Cadastro Mineiro (SCM): títulos e requerimentos", orgao="ANM", tipo_fonte="oficial",
         ...
         padroes=["dados/ANM/cadastro_mineiro/*.csv"], granularidade="processo × título — arquivo NACIONAL",
         formato_declarado="CSV separado por ponto e vírgula; Windows-1252", frequencia_atualizacao="diária (metadados ANM)",
    ...
    dict(source_id="SRC_ANM_PANORAMA_DERIVADO", nome_fonte="Panorama da Mineração em Goiás (análise derivada de dados ANM, extração de 04/08/2026)", orgao="interno (base ANM)", tipo_fonte="derivada",
         ...
         granularidade=f"várias ({len(openpyxl.load_workbook(f'{BASE}/dados/ANM_derivados_analises/Panorama_Mineracao_Goias_ANM.xlsx', read_only=True).sheetnames)} abas)", formato_declarado="XLSX", frequencia_atualizacao="—", periodo=lambda: "2007–2026 conforme a aba",
...
for f in FONTES:
    ...
    fmt_local = formato_csv_local(f["padroes"][0]) if f["padroes"] and f["padroes"][0].endswith(".csv") and "*" not in f["padroes"][0] else (
    ...
    fontes_rows.append(dict(
        ...
        status_uso="em uso" if cit else f.get("status_padrao", "catalogada, não citada"),
        ...
        data_arquivo_local=inf["data_arquivo_local"], periodo_coberto=f["periodo"](), granularidade=f["granularidade"],
...
for aba in ordem_abas:
    ...
    for c in cab:
        ...
        if aba in ABAS_MAQUETE:
            ...
            base = D.get(c)
            dsc = DA.get((aba, c)) or (base[0] if base else "")
...
DOC_DADOS = ["dados/LEIA-ME.md", "dados/*/*/metadados*.ods", "dados/ANM/cadastro_mineiro/mer-microdados-scm.pdf", "dados/SGB_GeoSGB/recmin/*.metadados.json"]
_cobertos = {os.path.normpath(a) for f in FONTES for p in f["padroes"] for a in glob.glob(f"{BASE}/{p}")}
_doc_dados = {os.path.normpath(a) for p in DOC_DADOS for a in glob.glob(f"{BASE}/{p}")}
_sem_src = sorted(os.path.relpath(os.path.join(r, n), BASE).replace(os.sep, "/") for r, _, fs in os.walk(f"{BASE}/dados") for n in fs
                  if os.path.normpath(os.path.join(r, n)) not in _cobertos | _doc_dados)
...
try:
    ...
    _rg = gpd.read_file(f"{BASE}/dados/SGB_GeoSGB/recmin/ocorrencias_recursos_minerais_GO.geojson")
