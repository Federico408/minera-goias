...
el('atlas-layer').onchange=render;for(const id of ['atlas-year-from','atlas-year-to']){
 el(id).oninput=yearLabel;el(id).onchange=()=>{yearLabel();render()}}el('atlas-mun').onchange=()=>selectMun(el('atlas-mun').value);el('mun-clear').onclick=()=>selectMun('');el('atlas-mineral').onchange=render;el('atlas-group').onchange=render;el('atlas-cat').onchange=render;el('atlas-search').oninput=()=>{limit=50;filterRows()};el('atlas-more').onclick=()=>{limit+=50;table()};el('atlas-reset').onclick=()=>map.fitBounds(bounds);el('atlas-export').onclick=exportRows;el('atlas-series').onchange=series;
...
if(layer==='cfem'&&!whole)el('atlas-scale-note').textContent=t('at.cfemYearNote');legend([[t('at.noneZero'),'#edf1f4'],...[scale.min,...scale.cuts].filter(v=>v!=null).map((v,i)=>[t('at.from')+' '+withUnit(v,unit),ramp[i]]),...(layer==='coefficient'?Object.entries(special):[])]);
