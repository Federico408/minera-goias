...
let packet,map,layers,municipalLayers=[],processPacket,processRecords,processLayers=[],loadingPromise,rows=[],visibleRows=[],limit=50,renderVersion=0,layer='cfem',catLayer='';
...
yearLabel();
el('atlas-layer').onchange=render;el('atlas-year').oninput=yearLabel;el('atlas-year').onchange=()=>{yearLabel();render()};el('atlas-mun').onchange=()=>selectMun(el('atlas-mun').value);el('mun-clear').onclick=()=>selectMun('');el('atlas-mineral').onchange=render;el('atlas-group').onchange=render;el('atlas-cat').onchange=render;el('atlas-search').oninput=()=>{limit=50;filterRows()};el('atlas-more').onclick=()=>{limit+=50;table()};el('atlas-reset').onclick=()=>map.fitBounds(bounds);el('atlas-export').onclick=exportRows;el('atlas-series').onchange=series;
...
}else if(layer==='projects'||layer==='occurrences'){const projects=layer==='projects',colors=projects?CLASS_COLORS:IMP_COLORS,field=projects?'classe':'importancia',label=projects?classLabel:importanceLabel,items=pointRows(layer),munName=new Map(packet.municipalities.map(m=>[m.code,m.name])),count=k=>items.filter(x=>x[field]===k).length;
if(catLayer!==layer){catLayer=layer;el('atlas-cat-label').textContent=projects?t('at.filterClass'):t('at.filterImportance');el('atlas-cat').innerHTML=`<option value="">${escape(t('at.allCategories'))}</option>`+Object.keys(colors).map(k=>`<option value="${escape(k)}">${escape(label(k))} (${escape(fmt(count(k)))})</option>`).join('')}
...
el('atlas-context').textContent=projects?t('at.projectsContext',{n:fmt(rows.length)}):t('at.occurrencesContext',{n:fmt(rows.length)});el('atlas-table-title').textContent=projects?t('at.projectsTitle'):t('at.occurrencesTitle');
...
if(layer==='cfem'){const source=cfem.get(normalize(m.name));value=year==='total'?m.cfem_total:source&&Object.hasOwn(source,year)?source[year]:null;note=year==='total'?t('at.accumShort'):source?(year==='2026'?t('at.partial2026'):year):t('at.noYearDetail');details=[[t('at.cfemCollected'),value==null?t('at.noRecord'):money(value)],[t('at.periodField'),note]]}
