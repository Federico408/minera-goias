...
el('atlas-layer').onchange=render;for(const id of ['atlas-year-from','atlas-year-to']){
 el(id).oninput=yearLabel;el(id).onchange=()=>{yearLabel();render()}}el('atlas-mun').onchange=()=>selectMun(el('atlas-mun').value);el('mun-clear').onclick=()=>selectMun('');el('atlas-mineral').onchange=render;el('atlas-group').onchange=render;el('atlas-cat').onchange=render;el('atlas-minfilter').onchange=render;el('atlas-search').oninput=()=>{limit=50;filterRows()};el('atlas-more').onclick=()=>{limit+=50;table()};el('atlas-reset').onclick=()=>map.fitBounds(bounds);el('atlas-export').onclick=exportRows;el('atlas-series').onchange=series;
...
el('atlas-context').textContent=(projects?t('at.projectsContext',{n:fmt(rows.length)}):t('at.occurrencesContext',{n:fmt(rows.length)}))+(minSel?' · '+minSel:'');el('atlas-table-title').textContent=projects?t('at.projectsTitle'):t('at.occurrencesTitle');
